...
function parseCronExpr(expr: string): string {
  ...
  const hasTime = minute !== "*" && hour !== "*" && !minute.includes("/") && !hour.includes("/");
  ...
  if (hasTime) {
    ...
    if (dayOfMonth === "*" && month === "*" && dayOfWeek !== "*") {
      if (dayOfWeek === "1-5" || dayOfWeek === "MON-FRI") return `Weekdays at ${time}`;
      if (dayOfWeek === "0,6" || dayOfWeek === "SAT,SUN") return `Weekends at ${time}`;
...
function CronCard({ job, agents, isAdmin }: { job: CronJob; agents: Agent[]; isAdmin: boolean }) {
    ...
    <div className="flex flex-col min-w-[280px] flex-1 rounded-xl border border-[var(--swarm-violet-dim)] bg-card/50 dark:swarm-glass">
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-1.5">
        {isLoading ? (
          ...
          jobs.map((job) => <CronCard key={job.id} job={job} agents={agents} isAdmin={isAdmin} />)
        )}
      </div>
    </div>
  );
