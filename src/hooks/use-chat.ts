...
export type PendingImageAttachment = {
  ...
  mimeType: string;
  fileName?: string;
  status?: "uploading" | "uploaded" | "failed" | "error";
  publicUrl?: string;
  error?: string;
};
...
function parseImageBlock(
  ...
  if (typeof raw.dataUrl === "string") {
    return { type: "image", dataUrl: raw.dataUrl, mimeType: blockMimeType };
  }

  if (typeof raw.url === "string") {
    return { type: "image", url: raw.url, mimeType: blockMimeType };
  }
...
function parseImageUrlBlock(
  ...
  return { type: "image", url: imageUrl.url };
}
...
export function useSessionChat(sessionKey: string) {
  ...
  function parseMessages(raw: Array<unknown>): ChatMessage[] {
    return raw.flatMap((m: unknown) => {
      ...
      return [{ role, content: extractText(m), timestamp: msg.timestamp ?? 0 }];
    });
  }
...
export const useChat = useSessionChat;
