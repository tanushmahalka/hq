...
import { type TaskCategory } from "../../shared/types.ts";
...
function resolvePluginConfig(config: unknown): HQMissionsConfig {
  ...
  if (direct.hqApiUrl || direct.hqApiToken) {
    return direct;
  }
  ...
  for (const id of preferredIds) {
    ...
    if (fromEntries.hqApiUrl || fromEntries.hqApiToken) return fromEntries;

    const fromShallowEntries = extractConfig(shallowEntries[id]);
    if (fromShallowEntries.hqApiUrl || fromShallowEntries.hqApiToken) {
      return fromShallowEntries;
    }

    const fromTopLevel = extractConfig(topLevel[id]);
    if (fromTopLevel.hqApiUrl || fromTopLevel.hqApiToken) return fromTopLevel;
  }
...
export default definePluginEntry({
  ...
  name: "HQ Missions",
  description:
    "Gives agents structured access to the HQ Mission Framework (missions, objectives, campaigns) and simple HQ task management.",
  register(api: PluginAPI) {
    ...
    if (!config.hqApiUrl || !config.hqApiToken) {
      api.logger?.warn?.(
        "[hq-missions] Missing hqApiUrl or hqApiToken - plugin inactive. Set config via pluginConfig or plugins.entries.<plugin-id>.config, or env HQ_API_URL/HQ_API_TOKEN.",
      );
      return;
    }
    ...
    api.registerTool({
      ...
      description:
        "Get the full mission briefing for the current agent, including all objectives, campaigns, and linked tasks.",
    ...
    api.registerTool({
      ...
      description:
        "Update a campaign's status (planned, active, paused, completed, failed).",
      parameters: Type.Object({
      ...
      async execute(_id, params) {
        ...
        return {
          content: [
            { type: "text", text: `Campaign ${campaignId} → ${params.status}` },
          ],
        };
      },
    });
    ...
    api.registerTool({
      ...
      description: "Report a metric update for an objective.",
      parameters: Type.Object({
      ...
      async execute(_id, params) {
        ...
        return {
          content: [
            { type: "text", text: `Objective metric updated: ${params.currentValue}` },
          ],
        };
      },
    });
    ...
    api.registerTool({
      ...
      description: "Record learnings or reflections for a campaign.",
      parameters: Type.Object({
        campaignId: numericIdSchema("Campaign ID"),
    ...
    api.registerTool({
      name: "create_mission",
      description: "Create a new mission for an agent.",
      parameters: Type.Object({
        agentId: Type.String({
    ...
    api.registerTool({
      ...
      description: "Create a new objective under a mission.",
      parameters: Type.Object({
        ...
        targetMetric: Type.Optional(Type.String()),
        targetValue: Type.Optional(Type.String()),
      ...
      async execute(_id, params) {
        ...
        const objective = await hq.call("custom.objective.create", {
          ...
          dueDate: toOptionalDate(params.dueDateIso),
    ...
    api.registerTool({
      ...
      description: "Create a new campaign under an objective.",
      parameters: Type.Object({
      ...
      async execute(_id, params) {
        ...
        const campaign = await hq.call("custom.campaign.create", {
          ...
          description: params.description as string | undefined,
          hypothesis: params.hypothesis as string | undefined,
          startDate: toOptionalDate(params.startDateIso),
          endDate: toOptionalDate(params.endDateIso),
          sortOrder: params.sortOrder as number | undefined,
        });
        return {
    ...
    api.registerTool({
      ...
      description: "List all tasks linked to a specific campaign.",
      parameters: Type.Object({
        campaignId: numericIdSchema("Campaign ID"),
      }),
      async execute(_id, params) {
        ...
        const linked = tasks.filter((task) => task.campaignId === campaignId);
        return {
    ...
    api.registerTool({
      name: "create_task",
      description: "Create a task in HQ.",
      parameters: Type.Object({
        ...
        dueDateIso: Type.Optional(Type.String({ description: "ISO date/time for due date" })),
    ...
    api.registerTool({
      name: "list_tasks",
      description: "List HQ tasks, optionally filtered by status.",
      parameters: Type.Object({
        status: Type.Optional(
          Type.Union([
            Type.Literal("todo"),
    ...
    api.registerTool({
      ...
      description: "Add a comment to a task in HQ.",
      parameters: Type.Object({
        taskId: Type.String(),
